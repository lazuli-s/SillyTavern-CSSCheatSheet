...
| **Custom CSS catalog** | `CUSTOM_CSS` array — each entry has a category name and a list of items, each with a `name`, a preview `html` snippet, and a `css` string containing the full copyable CSS code. Categories: Scrollbars, Chat Layout, Menus & Context Menus, Moonlit Echoes Theme, WorldInfoDrawer — Chips & Badges, WorldInfoDrawer — Dropdowns, WorldInfoDrawer — Bulk Edit, Tab Patterns |
